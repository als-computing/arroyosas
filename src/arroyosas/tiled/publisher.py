...
from ..schemas import SAS1DReduction

logger = logging.getLogger(__name__)


class Tiled1DResultsPublisher(Publisher):
    """Publisher that saves latent space vectors to a Tiled server."""

    def __init__(self, tiled_uri, tiled_api_key, root_segments=None):
        super().__init__()
        self.tiled_uri = tiled_uri
        self.tiled_api_key = tiled_api_key
        self.root_segments = root_segments or ["1D_live_results"]
        self.client = None
    ...
    async def publish(self, message):
        ...
        if not isinstance(message, SAS1DReduction):
            return
