...
class TiledEventSimulator:
    ...
    def list_available_runs(self):
        ...
        return [d.name for d in self.log_dir.iterdir() if d.is_dir() and any(d.glob("*.json"))]
    ...
    async def handle_client(self, websocket, path):
        ...
        try:
            ...
            if not runs:
                logger.error(f"No run directories found in {self.log_dir}")
                await websocket.send(json.dumps({"type": "error", "message": "No run directories found"}))
                return
    ...
    async def replay_run(self, websocket, run_id):
        ...
        for event_type in events_by_type:
            ...
            if event_type == "on_event":
                events_by_type[event_type].sort(
                    key=lambda e: e["callback_sequence"] if e["callback_sequence"] is not None else e["sequence"]
                )
            else:
