...
class TiledPoller:
    ...
    def _recent_page(self, root: Any, limit: int) -> list[tuple[str, Any]]:
        ...
        """
        try:
            # Tiled Tree sequences require step=-1 for negative-start slices.
            # This yields the last `limit` entries in reverse (newest-first).
            items = list(root[-limit:None:-1].items())
            items.reverse()  # restore oldest-first order for consistent processing
            return items
        except Exception as e:
